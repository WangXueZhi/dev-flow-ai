...
- [ ] `dev-flow report` writes `.devflow/artifacts/delivery-report.md` with source context, user stories, requirement constraints, acceptance criteria, stack, design tokens, API contracts, API data models, API error cases, API auth requirements, applied changes, review handoff notes, touched files, operation counts, line-count deltas, verification, visual checks, delivery readiness, and next actions.
