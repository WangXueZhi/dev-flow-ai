...
- Executor: supports `execute --dry-run`, writing AI-assisted or deterministic patch proposal documents without changing source files. Dry-run and patch-set prompts include the target profile and sampled source context so AI execution is grounded in likely repository files and existing code.
- Patch Applicator: supports `execute --apply`, validates strict JSON patch sets, enforces patch-set operation and payload limits, writes/replaces/deletes files, restores the apply backup automatically if a partial apply fails, and records `.devflow/artifacts/execution-log.json` with operation status, bytes written, replacements, and line-count deltas after successful applies.
