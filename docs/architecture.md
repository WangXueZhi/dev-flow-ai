...
- Status: reads `.devflow/artifacts/delivery-manifest.json` and prints either a compact delivery summary or raw manifest JSON, with applied operation counts, missing required visual text, visual layout issue counts, API state requirement counts, reviewer notes, source-context sampling evidence, verification remediation hints, remediation next actions, and optional CI gate exits for readiness attention, failed verification, and failed visual checks.
- Doctor: checks runtime and project readiness.
- GitHub Action: wraps safe `dev-flow deliver` for CI usage, requires explicit double confirmation for source-changing delivery, can require a clean git worktree before apply, can upload artifacts and job summaries with `always()`, and can fail CI from manifest-backed readiness, failed-verification, or failed-visual gates.
