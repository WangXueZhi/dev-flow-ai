...
- Stack Detector: detects package manager, runtime, frontend frameworks, build and quality tools, styling, testing, scripts, source directories, and config files from dependency metadata plus common frontend conventions such as Next.js, Vue, Nuxt, Svelte, Angular, Astro, Tailwind, Biome, ESLint, Prettier, Vitest, Playwright, Cypress, and Jest config files.
