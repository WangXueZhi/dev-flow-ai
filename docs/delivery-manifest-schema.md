...
- Delivery counts for acceptance criteria, open questions, delivery risks, design tokens, API state requirements, changed files, verification commands, visual evidence, and applied operations.
...
- Visual screenshots, required text checks, and layout issue details.
...
Verification command entries may include `outputExcerpt`, `remediation`, and `remediationPlan` when a command fails. Excerpts are bounded summaries intended for CI logs, dashboards, and follow-up agents; remediation hints provide command-category follow-up guidance, while `remediationPlan` adds a category, ordered next actions, and related artifact references for downstream automation. Use the full verification report when complete command output is required.
