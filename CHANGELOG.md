...
- Added applied operation details to delivery manifests and surfaced applied operation counts in local status and GitHub Actions job summaries.
...
- Added `dev-flow status --fail-on-attention`, `--fail-on-failed-verification`, `--fail-on-failed-visual`, and `--fail-on-missing-artifacts` for CI delivery gates backed by the manifest.
...
- Updated external release diagnostics to derive the default package name, release tag, and repository from `package.json` and bound each external command with a timeout.
